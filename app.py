...
# --- CARICAMENTO DATI (CATEGORIE E BUDGET) ---
...
with tab1:
    ...
    if not df_mail.empty:
        ...
        st.subheader("💰 Entrate Mail")
        df_e = st.data_editor(df_mail[df_mail["Tipo"]=="Entrata"], column_config={"Categoria": st.column_config.SelectboxColumn(options=CAT_ENTRATE)}, use_container_width=True, key="ed_e")
        st.subheader("💸 Uscite Mail")
        df_u = st.data_editor(df_mail[df_mail["Tipo"]=="Uscita"], column_config={"Categoria": st.column_config.SelectboxColumn(options=CAT_USCITE)}, use_container_width=True, key="ed_u")
    ...
    df_man = st.data_editor(st.session_state["df_manual_entry"], num_rows="dynamic", use_container_width=True, key="ed_man")

    if st.button("💾 SALVA TUTTO NEL CLOUD", type="primary", use_container_width=True):
        da_unire = [df_cloud]
        if 'ed_e' in st.session_state: da_unire.append(st.session_state["ed_e"]["edited_rows"]) # Nota: Logica semplificata qui per spazio
        # Per brevità usiamo un approccio diretto:
        final_save = pd.concat([df_cloud, df_e if not df_e.empty else None, df_u if not df_u.empty else None, df_man[df_man["Importo"]>0]], ignore_index=True)
        final_save["Data"] = pd.to_datetime(final_save["Data"]).dt.strftime("%Y-%m-%d")
        conn.update(worksheet="DB_TRANSAZIONI", data=final_save)
...
with tab2:
    ...
    # Calcolo Reale vs Budget
    df_curr = df_cloud[(df_cloud["Data"].dt.month == [k for k,v in map_mesi.items() if v==mese_sel][0])]
    reale_u = df_curr[df_curr["Tipo"]=="Uscita"].groupby("Categoria")["Importo"].sum().reset_index()
    ...
    with col_g1:
        st.plotly_chart(px.pie(reale_u, values='Importo', names='Categoria', title=f"Spese Reali {mese_sel}", hole=.4), use_container_width=True)
    with col_g2:
        st.dataframe(comp.style.format("{:.2f} €").applymap(lambda x: 'color:red' if x<0 else 'color:green', subset=['Delta']), use_container_width=True)

# --- TAB 3: STORICO ---
with tab3:
    edited_db = st.data_editor(df_cloud, num_rows="dynamic", use_container_width=True)
    if st.button("🔄 Aggiorna Database"):
        conn.update(worksheet="DB_TRANSAZIONI", data=edited_db)
