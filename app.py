...
# --- CARICAMENTO BUDGET (DA FOGLIO DB_BUDGET) ---
@st.cache_data(ttl=60)
def get_budget_data():
    try:
...
with tab1:
    ...
    if not df_view_entrate.empty:
        edited_entrate = st.data_editor(df_view_entrate, column_config={"Categoria": st.column_config.SelectboxColumn(options=CAT_ENTRATE, required=True), "Tipo": st.column_config.Column(disabled=True), "Data": st.column_config.DateColumn(format="YYYY-MM-DD", required=True), "Importo": st.column_config.NumberColumn(format="%.2f €")}, key="edit_entrate_mail", use_container_width=True)
    else:
        st.info("Nessuna nuova entrata.")

    st.markdown("##### 💸 Nuove Uscite")
    if not df_view_uscite.empty:
        edited_uscite = st.data_editor(df_view_uscite, column_config={"Categoria": st.column_config.SelectboxColumn(options=CAT_USCITE, required=True), "Tipo": st.column_config.Column(disabled=True), "Data": st.column_config.DateColumn(format="YYYY-MM-DD", required=True), "Importo": st.column_config.NumberColumn(format="%.2f €")}, key="edit_uscite_mail", use_container_width=True)
    else:
    ...
    edited_manual = st.data_editor(st.session_state["df_manual_entry"], num_rows="dynamic", column_config={"Categoria": st.column_config.SelectboxColumn(options=sorted(CAT_USCITE + CAT_ENTRATE), required=True), "Tipo": st.column_config.SelectboxColumn(options=["Uscita", "Entrata"], required=True), "Data": st.column_config.DateColumn(format="YYYY-MM-DD", required=True), "Importo": st.column_config.NumberColumn(format="%.2f €")}, key="edit_manual", use_container_width=True)
    ...
    if st.button("💾 SALVA TUTTO NEL CLOUD", type="primary", use_container_width=True):
        ...
        if da_salvare:
            ...
            st.session_state["df_mail_found"] = pd.DataFrame(); st.session_state["df_manual_entry"] = pd.DataFrame(); st.session_state["df_mail_discarded"] = pd.DataFrame()
            st.balloons(); st.success("✅ Tutto salvato correttamente!"); st.rerun()

# ==========================================
# TAB 2: REPORT & PROSPETTI + BUDGET + GRAFICI
# ==========================================
with tab2:
    if df_cloud.empty:
        ...
        # Filtro Anno e Mese laterale
        col_f1, col_f2 = st.columns(2)
        with col_f1: anno_sel = st.selectbox("📅 Seleziona Anno", sorted(df_analysis["Anno"].unique(), reverse=True) if not df_analysis.empty else [2026])
        with col_f2: mese_sel_nome = st.selectbox("📆 Seleziona Mese Analisi", list(map_mesi.values()), index=datetime.now().month-1)
        ...
        k1.metric("Entrate Totali (Anno)", f"{tot_entrate:,.2f} €", border=True)
        k2.metric("Uscite Totali (Anno)", f"{tot_uscite:,.2f} €", delta_color="inverse", border=True)
        k3.metric("Saldo Netto", f"{saldo:,.2f} €", border=True)
        
        # --- SEZIONE BUDGET (PREVENTIVO) ---
        ...
        if not df_budget.empty:
            ...
            # Logica SALDO INIZIALE: solo per Gennaio
            ...
            k4.metric("Ancora in Tasca (Mese)", f"{(budget_tot_mese - speso_tot_mese):,.2f} €", delta=f"{(budget_tot_mese - speso_tot_mese):,.2f} €")
        ...
        # --- SOTTOTAB STORICHE ---
        sub_t1, sub_t2 = st.tabs(["🗓 Prospetti Mensili", "📈 Trend Annuale"])
        with sub_t1:
